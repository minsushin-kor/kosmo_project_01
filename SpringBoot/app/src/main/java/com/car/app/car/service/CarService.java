...
public class CarService {
     ...
     *
     * @param loginId     로그인한 사용자 ID (loginId)
     * @param authorities 로그인한 사용자의 보안 역할 권한 목록
     * @param request     차량 스펙 및 이미지 정보가 포함된 DTO
     * @return 저장 및 경매 연계가 완료된 Car 엔티티
     */
    @Transactional
    public Car registerCar(String loginId, Collection<? extends GrantedAuthority> authorities,
        ...
        if (isMember) {
            // 회원인 경우: 로그인 아이디로 회원 엔티티 로드
            memberOwner = memberRepository.findByLoginId(loginId)
                    .orElseThrow(() -> new IllegalArgumentException("존재하지 않는 회원 계정입니다."));
        } else if (isDealer) {
            // 딜러인 경우: 로그인 ID로 딜러 엔티티 로드
            dealerOwner = dealerRepository.findByLoginId(loginId)
     ...
     */
    @Transactional
    public void deleteCar(Long carId, String loginId) {
        Car car = carRepository.findById(carId)
    ...
    /**
     * 현재 로그인한 일반회원 또는 딜러가 직접 등록한 차량 목록을 조회합니다.
     */
    @Transactional(readOnly = true)
    public List<CarDto.Response> getMyCars(
            String loginId,
        ...
        boolean isDealer = authorities.stream()
                .anyMatch(authority -> authority.getAuthority().equals("ROLE_DEALER"));

        List<Car> myCars;

        if (isMember) {
            myCars = carRepository
                    .findAllByMemberLoginIdOrderByCreatedAtDesc(
                            loginId);
        } else if (isDealer) {
            myCars = carRepository
                    .findAllByDealerLoginIdOrderByCreatedAtDesc(
                            loginId);
        } else {
            throw new SecurityException(
                    "차량 목록을 조회할 권한이 없습니다.");
        ...
        /*
         * images는 지연 로딩 관계이므로 트랜잭션 안에서
         * 미리 초기화하여 응답 변환 시 오류를 방지합니다.
         */
        myCars.forEach(car -> car.getImages().size());
    ...
    /**
     * Car 엔티티를 CarDto.Response 포맷으로 매핑하며, 일반회원 차량의 경우 auctions 테이블을 조인하여 경매
     * 정보(auctionId, startTime, endTime, auctionStatus, bidCount)를 함께 반환합니다.
     */
