...
DEFAULT_MODEL_PATH = MODEL_DIR / "vehicle_condition_catboost_model.pkl"

FEATURES = ["year", "make", "model", "odometer"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the dealer vehicle-condition CatBoost model."
    )
...
def load_training_data(data_path: Path) -> pd.DataFrame:
    ...
    # CatBoost can learn an explicit missing category for text columns.
    data[CATEGORICAL_FEATURES] = (
        data[CATEGORICAL_FEATURES].fillna("Unknown").astype(str)
    )
    data = data.dropna(subset=["year", "odometer", TARGET, DATE_COLUMN])
    data = data[data[TARGET].between(1.0, 5.0)]
...
def regression_metrics(actual: np.ndarray, predicted: np.ndarray) -> dict[str, float]:
    ...
    return {
        ...
        "rmse": float(mean_squared_error(actual, clipped) ** 0.5),
        "r2": float(r2_score(actual, clipped)),
        "within_0_5_rate": float(np.mean(np.abs(actual - clipped) <= 0.5)),
    }
...
def train_with_cross_validation(
    ...
    for fold_number, (fit_indices, validation_indices) in enumerate(
        ...
        print(
            ...
            f"R2={metrics['r2']:.4f}, "
            f"within_0.5={metrics['within_0_5_rate'] * 100:.2f}%"
        )
...
def main() -> None:
    ...
    cv_summary = {
        ...
        for metric in ["mae", "rmse", "r2", "within_0_5_rate"]
    }
    bundle = {
        "model": final_model,
    ...
    print(
        ...
        f"within_0.5={holdout_metrics['within_0_5_rate'] * 100:.2f}%"
    )
